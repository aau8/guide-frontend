...
if (new Date().getTime() - localStorage.getItem('lastTimeCheckValidHTML') > 1/5*60000 || (localStorage.getItem('dataValid') == null && localStorage.getItem('dataValid') == '')) {
    ...
    for (let i = 0; i < validBtnElems.length; i++) {
        ...
        checkOnError(validBtn.href)
            .then(notValid => {
                ...
                dataValid.push({ cardId: card.id, valid })
    ...
    for (let i = 0; i < dataValid.length; i++) {
        ...
        const validBtn = document.getElementById((elem.cardId)).querySelector('.sc-card__validator')
...
async function checkOnError(url) {
    ...
    return await fetch(url)
        .then(data => data.json())
        .then(json => json.messages.length != 0 ? true : false)
}